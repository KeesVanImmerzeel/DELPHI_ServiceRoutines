...
type Tpoint3D = record x,y,z : single; end;
     Tfacetype = record point1, point2, point3 : Tpoint3D;
...
procedure xy3dsetcolors(cs : array of Tcolor);
...
procedure xy3dsetlabels(xl,yl,zl,xrl,yrl,zrl:string;xm,ym,zm:integer;zv:boolean); overload;
...
var xmi,xma : single;             {waarden X-as}
    ...
    revcol : boolean;             {kleuren moeten omgekeerd}
...
var  datap,crsdp : datapointer;         {pointer naar data}
...
begin
  xlabel := 'X'; xrlabel := '-X'; xlmode := 0;
  ylabel := 'Y'; yrlabel := '-Y'; ylmode := 0;
  zlabel := 'Z'; zrlabel := '-Z'; zlmode := 0;
  zlvert := false; labels3d := false;
end;
...
function hcolor(h:single;col:Tcolor):integer; {XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX}
var t : integer;
begin
...
var i : integer; {NB: kopie in 4D zonder trans}
begin
...
begin
  ...
  if (m=4) then {zlabel in kolom}
     begin
       ...
       xytextangle(-1,s,x0,y0,0,0,8+16,h);
     end
  else if (m=2) then xytextangle(-1,s,x0,y0,0,0,8,h)
  else
  with xycanvas do
    begin
      if (m=0) or (x1=x2) then x0 := x0 - textwidth(s) div 2
      else if (x1<x2) then x0 := x0 - textwidth(s);
      y0 := y0 - textheight(s) div 2;
      if (dowmf) then
        begin checkwmffont; textwmfalign(0); textwmfout(x0,y0,s); end
      else textout(x0,y0,s);
    end;
end;
...
begin
  ...
  if (m=2) then
    if (kijkhoogte>=0) or zas then xytextangle(-1,s,x0,y0,0,1,8,h)
                              else xytextangle(-1,s,x0,y0,0,-1,8,h)
  else
  with xycanvas do begin

  if zas then
    begin
      if (x2>x1) then x0 := x0-tw else x0 := x0;
      y0 := y0-th2;
    end
  else
    begin
      if (l=0) then begin {x0 := x0-tw2;} y0 := y0+th; end else
...
procedure text3(s:string;x1,y1,x2,y2,d,m:integer;tic:boolean); {XXXXXXXXXXXXXXX}
  {tekent namen in verlengde van lijn}
var x0,y0,h : integer;
    l,f : single;
begin
  if (m<2) then begin text1(s,x1,y1,x2,y2,d,0,m); exit; end;
  if not tic then d := d - xycanvas.textwidth('0');
  l := sqrt(sqr(x2-x1) + sqr(y2-y1)); if (l=0) then f := 0 else f := d/l;
  x0 := x1 - round(f*(x2-x1)); y0 := y1 - round(f*(y2-y1));
  if tic and (m<8) then xyline(-1,x0,y0,x1,y1);

  if (s='') then exit;
  h := slope(x2,y2,x1,y1); if (h>90) or (h<-90) then h := h + 180;
  if (x1>x2) then xytextangle(-1,s,x0,y0, 1,0,8,h)
             else xytextangle(-1,s,x0,y0,-1,0,8,h)
end;
...
var i,j,n : integer; t : single; first : boolean;
begin
  ok := true; surf := false; graphs3d[igraph].dp := nil;
...
begin
  ...
  with graphs3d[igraph] do with xycanvas do if scl then
    begin
     if not contzlog then y := ys1 + round((z2-h)/(z2-z1)*(ys2-ys1))
           else y := ys1 + round(ln(z2/h)/ln(z2/z1)*(ys2-ys1));
...
{bepaalt kleur bij bepaalde hoek}
var n : integer;
begin
  if (h<zmi) then n := 0 else
  if (h>zma) then n := 255 else
  if contzlog then n := round((ln(h)-lzmi)/(lzma-lzmi)*255)
       else n := round((h-zmi)/(zma-zmi)*255);
  if crev then n := 255 - n;
  case cmode of
    1 : if odd(n div cres) then result := 0 else result := $ffffff;
   2,3 : begin  n := (n div cres) * cres; result := colors[n]; end;
   else result := 0;
  end
end;
...
var t,y,cl,p,clh,clh0,c0,y0,yy,pw : integer;
    h : single;
    oldlog : boolean;
begin
  ...
  if contzlog then
  case hmode of
   0 : xyyaxis(ascol,zmi,zma,  0,0,txt,p,false,true,false);
   1 : xyyaxis(ascol,1,zma/zmi,0,0,txt,p,false,true,false);
   2 : xyyaxis(ascol,zmi/zma,1,0,0,txt,p,false,true,false);
  end
  else
  case hmode of
   0 : xyyaxis(ascol,zmi,zma,  0,0,txt,p,false,false,false);
   1 : xyyaxis(ascol,0,zma-zmi,0,0,txt,p,false,false,false);
   2 : xyyaxis(ascol,zma-zmi,0,0,0,txt,p,false,false,false);
  end;
  ...
  case cmode of
   ...
   1,2,3 : begin
        y0 := y1; xypen.Style :=  psclear;
        pw := xypen.width; xypen.width := 1;
        for y := y1 to y2 do
          begin h := zmi + (y2-y)/(y2-y1)*(zma-zmi);
            clh := color(h);
            if (y=y1) then clh0 := clh else
            if (clh<>clh0) or (y=y2) then
             begin
               if (y=y2) then yy := y2+1 else yy := y;
               c0 := reversecolor(clh0);
               if dowmf then
               xyrectangle(0,c0,x1-2,y0-1,x2+0  ,yy-0) else
               xyrectangle(0,c0,x1-1,y0  ,x2+1+1,yy+1);
               clh0 := clh; y0 := y;
             end;
         end;
         xypen.Style :=  pssolid; xypen.width := pw;
        end;
...
begin {------------------------------------------------------------------------}
  ...
  if (opt and 1 = 0 ) and scale and (h>=zmi) and (h<=zma) then
    begin
      if not contzlog then y := sy1 + round((zma-h)/(zma-zmi)*(sy2-sy1))
         else y := sy1 + round((lzma-ln(h))/(lzma-lzmi)*(sy2-sy1));
...
procedure xy3dsetcolors(cs : array of Tcolor); {XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX}
var i,nc,ti,tf,cl:integer;  t : single;
begin
  ...
  colors[0] := cs[0]; colors[255] := cs[nc-1];
  for i := 1 to 254 do
    begin
      t := i*(nc-1)/255; ti := trunc(t); tf := round((t-ti)*256);
...
type regel = array[0..8000] of cardinal;
     pregel = ^regel;
var x,y,lx,ly,refr,pw,n,n1,n2,p : integer;
    ...
    begin
      ...
      for x := 0 to xp2-xp1 do
        begin
          ...
          begin
            if (cmode=2) then cl := color(datap^[nx,ny])
            else begin
            ...
            then cl := reversecolor(backcolor) else
            if (cmode<4) then
               begin h := (h1+h2+h3+h4)/4; cl := color(h); end
            else
              begin
...
begin {------------------------------------------------------------------------}
  ...
  zmi := hmin; zma := hmax;
  if (zmi=0) and (zma=0) then
     begin zmi := srf.zmi; zma := srf.zma; end;
  if (zmi<=0) or (zma<0) then contzlog := false;
  if (zmi=zma) then
    if contzlog then begin zmi := zmi-1; zma := zma+1; end
                else begin zmi := zmi*1.1; zma := zma/1.1; end;
  if contzlog then begin lzmi := ln(zmi); lzma := ln(zma); end;
  ...
  with graphs3d[igraph] do
    begin z1 := zmi; z2 := zma; hm := hmode; drm := drmode;
...
var xps, yps : array of array of smallint;
    ...
    dik,foto,nogrid,trans,back,lyncol,vlakcol : boolean;
    dh1,dh2,dh,t : single;
    col : Tcolor;
...
var h,dx,dy,h1,h2,h3,h4,h12,h23,h34,h14,h13,h24,hm,xm,ym : single;
    b : boolean;
    pw : integer;
...
begin { - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -}
  ...
  if nosplit or (flatno and (abs(h13-h24)<(zma-zmi)/1000)) then
    begin
      ...
      else if vlakcol then xybrush.color := heightcol((h1+h2+h3+h4)/4);
      if nogrid then begin xypen.width := 1; xypen.color := xybrush.color; end;
      if (mode>4) then
        if trans then transpolygon(-1,-1,point4) else xypolygon(-1,-1,point4);
      ...
      if dowmf then
        begin
          ...
          xypen.color := heightcol((h1+h2)/2); checkwmfpen; drawwmf(xps[x+1,y],yps[x+1,y]);
          xypen.color := heightcol((h2+h3)/2); checkwmfpen; drawwmf(xps[x+1,y+1],yps[x+1,y+1]);
          xypen.color := heightcol((h3+h4)/2); checkwmfpen; drawwmf(xps[x,y+1],yps[x,y+1]);
          xypen.color := heightcol((h4+h1)/2); checkwmfpen; drawwmf(xps[x,y],yps[x,y]);
        end
      else with xycanvas do
        begin
          moveto(xps[x,y],yps[x,y]);
          xypen.color := heightcol((h1+h2)/2); lineto(xps[x+1,y],yps[x+1,y]);
          xypen.color := heightcol((h2+h3)/2); lineto(xps[x+1,y+1],yps[x+1,y+1]);
          xypen.color := heightcol((h3+h4)/2); lineto(xps[x,y+1],yps[x,y+1]);
          xypen.color := heightcol((h4+h1)/2); lineto(xps[x,y],yps[x,y]);
        end;
      xypen.width := pw;
   end
  ...
  for x := 2 to nx do
    begin
      ...
      if hasempty and ((t=empty) or (tt=empty)) then
        ...
        begin
         if lyncol then xypen.color := heightcol((datap^[x,y]+datap^[x-1,y])/2);
  ...
  for x := 1 to nx do
    begin
      ...
      if not ( hasempty and ((t=empty) or (tt=empty)) ) then
        begin
          if lyncol then xypen.color := heightcol((t+tt)/2);
...
begin
  ...
  for x := 1 to nx do for y := 1 to ny do
    begin position(datap^[x,0],datap^[0,y],datap^[x,y],xp,yp);
    ...
    for x := 1 to nx do for y := 1 to ny do
      begin
        t := datap^[x,y]; if not hasempty or (t<>empty) then
          begin
           if not lyncol then col := col1 else col := heightcol(t);
...
begin
  ...
  labels3d := lbl3d; xlmode := xm; ylmode := ym; zlmode := zm;
...
procedure xy3dsetlabels(xl,yl,zl,xrl,yrl,zrl:string;xm,ym,zm:integer;zv:boolean);
begin
   if zv then xy3dsetlabels(xl,yl,zl,xrl,yrl,zrl,false,xm,ym,zm,1)
         else xy3dsetlabels(xl,yl,zl,xrl,yrl,zrl,false,xm,ym,zm,0);
end;

procedure init3dlabels(t,h:integer);
var f,f0,f1,f2,ff:single;
    l : integer;
    ok : boolean;
begin
  f0 := 1;
...
var i,t1,t2,t3,tz,nd,l,ml,zh,dmode,d,cw,ch: integer;
...
begin
  ...
  if labels3d or odd(zlmode) then d := 2*ch else d := 5*cw;
  if odd(oct) then
  ...
  if labels3d then verdeling3d(zma-zmi,round((zma-zmi)*pixz3d),d,gr,nd,stm)
    else begin l := round(sqrt(sqr(x2-x1)+sqr(y2-y1)));
    verdeling3d(zma-zmi,l,d,gr,nd,stm); end;
  p := gr*trunc(zmi/gr-1); ml := 0;
  repeat
    position(xx[t1],yy[t1],p,xt,yt); position(xx[t2],yy[t2],p,x0,y0);
    if  ( (yt<=y1) and (yt>=y2) ) or ( (yt>=y1) and (yt<=y2) ) then
      begin
        if dotext then str(p:1:nd,s) else s := '';
        if (length(s)>ml) then ml := length(s);
        if labels3d then text3d(s,xx[tz],yy[tz],p,zlmode+9,false,oct,ml,zlvert) else
        if (zlmode=2) and zlvert then text3(s,xt,yt,x0,y0,cw,zlmode+dmode,true)
              else text2(s,xt,yt,x0,y0,cw,zh,zlmode+dmode,true);
  ...
  if doxyz and dotext then
    if labels3d then text3d(zlabel,xx[tz],yy[tz],zmid,zlmode+9,true,oct,ml,zlvert)
    else begin
      position(xx[t1],yy[t1],zmid,xt,yt); position(xx[t2],yy[t2],zmid,x0,y0);
      if not zlvert then      text1(zlabel,xt,yt,x0,y0,cw*(ml+3),zh,zlmode)
      else if (zlmode=2) then text1(zlabel,xt,yt,x0,y0,cw*(ml+3),zh,4)
                         else text1(zlabel,xt,yt,x0,y0,cw*(ml+3),-90,4);
    end;
end;
...
var t1,t2,t3,xh,cw,ch,d,nd,l,ml,i : integer;
    x0,y0,x1,y1,x2,y2,xt,yt : integer;
    f,r1,r2,gr,p,pp : single;
    s : string;
begin
  cw := xycanvas.textwidth('0'); ch := xycanvas.textheight('0');
  if odd(zlmode) then d := 2*ch else d := 5*cw;
  case oct of
    2,3 : t1 := 1; 4,5 : t1 := 2;
    6,7 : t1 := 3; 8,1 : t1 := 0;
  end;
  ...
  if labels3d then verdeling3d(r2-r1,round((r2-r1)*pixx3d),d,gr,nd,stm)
   else begin l := round(sqrt(sqr(x2-x1)+sqr(y2-y1))/2);
    verdeling3d(r2-r1,l,6*cw,gr,nd,stm); end;
  ...
  repeat
    ...
    begin
      ...
      if oct in [1,4,5,8] then begin
        if labels3d then text3d(s,pp,-yy[t],zz[h],xlmode+1,false,oct,ml,zlvert)
        else
          begin
             position(pp,-yy[t],zz[h],xt,yt); position(pp,yy[t],zz[h],x0,y0);
             text2(s,xt,yt,x0,y0,ch,xh,xlmode,false); end; end
      else begin
        if labels3d then text3d(s,-xx[t],pp,zz[h],xlmode+5,false,oct,ml,zlvert)
        else
  ...
  if doxyz and dotext then
   if oct in [1,4,5,8] then
    if labels3d then text3d(xlabel,0,-yy[t],zz[h],xlmode+1,true,oct,ml,zlvert)
    else begin position(0,-yy[t],zz[h],xt,yt); position(0,yy[t],zz[h],x0,y0);
       text1(xlabel,xt,yt,x0,y0,cw*(ml+4),xh,xlmode); end
   else
    if labels3d then text3d(xlabel,-xx[t],0,zz[h],xlmode+5,true,oct,ml,zlvert)
...
var t,h,t1,t2,t3,h2,x1,x2,y1,y2,xt,yt,x0,y0,ch,cw,l,nd,ml,i,j,f,stm : integer;
    xh,dmode,m : integer;
    s : string;
    ticr,ticr2, ticz, ticz2, f0, ff, f2, f1 : single;
...
begin
  ...
  ticr := ch/pixx3d; ticz := ch/pixz3d;
  ticr2 := cw/pixx3d; ticz2 := cw/pixz3d;
  xylinewidth(1);
  polartype := 0;
  ...
  if labels3d then init3dlabels(t,h);
  ...
  if (mode<3) then
    begin
      ...
      if doxyz and (mode<3) and dotext then text3(s,x1,y1,x2,y2,ch,ylmode,false);
   ...
   begin
      ...
      if doxyz and dotext then
        begin
         ...
         if labels3d then text3d(s,0,yy[t],zz[1-h],ylmode+13,true,oct,ml,zlvert)
          else text3(s,x1,y1,x2,y2,ch,ylmode,false);
        end;
      ...
      if doxyz and dotext then
        begin
         ...
         if labels3d then text3d(s,xx[t],0,zz[1-h],13+ylmode,true,oct,ml,zlvert)
         else text3(s,x1,y1,x2,y2,ch,ylmode,false);
        end;
...
begin
    ...
    begin
      ...
      if dotext then if doxyz then if (mode>2) then
        begin
          if (oct in [1,2,7,8]) xor polrev then s := '90' else s := '270';
          if labels3d then text3d(s,0,yy[t],zz[1-h],ylmode+13,true,oct,ml,zlvert)
          else text3(s,x1,y1,x2,y2,ch,ylmode,false) {<<XZ}
        end
      else text3(s,x1,y1,x2,y2,ch,zlmode,false); {Z,XZ}
      ...
      if doxyz and dotext then if (mode>2) then
        begin
          if oct in [1,2,3,4] then s := '0' else s := '180';
          if labels3d then text3d(s,xx[t],0,zz[1-h],13+ylmode,true,oct,ml,zlvert)
          else text3(s,x1,y1,x2,y2,ch,ylmode,false); {<<YZ}
        end
      else text3(s,x1,y1,x2,y2,ch,zlmode,false); {Z-YZ}
    end
  else
    begin {een Z lijn}
      position(xx[t],yy[t],zz[h],x2,y2); position(xx[t],yy[t],zz[1-h],x1,y1);
      if doline then xyline(-1,x2,y2,x1,y1);
      if doxyz and (mode<3) and dotext then text3(s,x1,y1,x2,y2,ch,zlmode,false);
    end;
  ...
  if (mode>1) then {lijnen in midden}
    begin
      ...
      if doxyz and (mode<3) and dotext then text3(s,x1,y1,x2,y2,ch,ylmode,false); {<<XY}
...
var t,h,t1,t2,t3,h2,x1,x2,y1,y2,xt,yt,x0,y0,ch,cw,l,nd,ml,stm : integer;
    xh,yh,zh,i,m,mm,dmode,tz : integer;
    s : string;
...
begin
  ...
  if (mode in [1,2]) or ( (frcol>=0) and dothick) then {teken (dikke) XYZ assen}
    begin
      ...
      if doxyz and (mode<3) and dotext then text3(s,x1,y1,x2,y2,ch,m,false);
  ...
  xh := slope(x2,y2,x1,y1);
  if labels3d then
   if odd(xlmode) then verdeling3d(xma-xmi,round((xma-xmi)*pixx3d),2*ch,gr,nd,stm)
                  else verdeling3d(xma-xmi,round((xma-xmi)*pixx3d),5*cw,gr,nd,stm)
    else begin l := round(sqrt(sqr(x2-x1)+sqr(y2-y1)));
    verdeling3d(xma-xmi,l,4*cw,gr,nd,stm); end;
  p := gr*trunc(xmi/gr-1); ml := 0;
  repeat
    position(p,yy[t1],zz[h],xt,yt); position(p,yy[t3],zz[h],x0,y0);
    if  ( ( (yt<=y1) and (yt>=y2) ) or ( (yt>=y1) and (yt<=y2) ) ) and
        ( ( (xt<=x1) and (xt>=x2) ) or ( (xt>=x1) and (xt<=x2) ) ) then
      begin
        if dotext then str(p:1:nd,s) else s := '';
        if (length(s)>ml) then ml := length(s);
        if labels3d then text3d(s,p,yy[t1],zz[h],xlmode+1,false,oct,ml,zlvert) else
        text2(s,xt,yt,x0,y0,ch,xh,xlmode+dmode,false);
        if (mode=4) and doline then
          begin
            xyline(-1,xt,yt,x0,y0);
  ...
  if dotext and doxyz then
    if labels3d then text3d(xlabel,xmid,yy[t1],zz[h],xlmode+1,true,oct,ml,zlvert) else
    begin
      position(xmid,yy[t1],zz[h],xt,yt); position(xmid,yy[t3],zz[h],x0,y0);
      text1(xlabel,xt,yt,x0,y0,cw*(ml+5),xh,xlmode);
    end;
  end;
  ...
  if labels3d then
   if odd(ylmode) then verdeling3d(yma-ymi,round((yma-ymi)*pixy3d),2*ch,gr,nd,stm)
     else verdeling3d(yma-ymi,round((yma-ymi)*pixy3d),5*cw,gr,nd,stm)
    else begin l := round(sqrt(sqr(x2-x1)+sqr(y2-y1)));
    verdeling3d(yma-ymi,l,4*cw,gr,nd,stm); end;
  p := gr*trunc(ymi/gr-1); ml := 0;
  repeat
    position(xx[t1],p,zz[h],xt,yt); position(xx[t3],p,zz[h],x0,y0);
    if  ( ( (yt<=y1) and (yt>=y2) ) or ( (yt>=y1) and (yt<=y2) ) ) and
        ( ( (xt<=x1) and (xt>=x2) ) or ( (xt>=x1) and (xt<=x2) ) ) then
      begin
        if dotext then str(p:1:nd,s) else s := '';
        if (length(s)>ml) then ml := length(s);
        if labels3d then text3d(s,xx[t1],p,zz[h],ylmode+5,false,oct,ml,zlvert) else
        text2(s,xt,yt,x0,y0,ch,yh,ylmode+dmode,false);
        if (mode=4) and doline then
          begin
            xyline(-1,xt,yt,x0,y0);
  ...
  if doxyz and dotext then
    if labels3d then text3d(ylabel,xx[t1],ymid,zz[h],ylmode+5,true,oct,ml,zlvert) else
    begin
      position(xx[t1],ymid,zz[h],xt,yt); position(xx[t3],ymid,zz[h],x0,y0);
      text1(ylabel,xt,yt,x0,y0,cw*(ml+5),yh,ylmode);
    end;
  end;
...
procedure xy3dinitruler(cl:Tcolor;xp,yp,j:integer); {XXXXXXXXXXXXXXXXXXXXXXXXXX}
 {initieert lineaal}
begin rul3d := true; xyinitruler(cl,xp,yp,j,0); end;
