...
begin
   ...
   xyxaxis(clblack,1,100,0,0,'X-axis',gridoff,logscale,fixed);
   ...
   xylinewidth(1);
   xymove(0,0); for i := 1 to 9 do
     begin xydraw(i*10,i*10); xysymbol(2,8,1); end;
  ...
  { xystartgraph(50,100,4,65,70,10,30,20,clipon);
   ...
   xyyaxis(clblack,0,180,0,0,'Results',1,gridon,lin,fixed);
   if logscale then
     begin
   ...
   xytextangle(clblack,'XYGRAPH|(C) 2002',paintbox1.width-10,
     round(paintbox1.height*0.82),0,1,3,-90);
...
begin
  ...
  if checkbox1.checked then
    begin
      ...
      xytimeaxis(clblack,d0-dd,d0+dd,'date',0,0,0,0,1,false,tm);
    end
  else
    begin
      dmin := 1/24/60/60;
      xystartgraph(0,100,0,40,30,20,30,50,clipon);
      xytimeaxis(clblack,d0-dd,d0+dd,'date|time and date '+ystring,-1,1,1,1,2,false,tm);
    end;
